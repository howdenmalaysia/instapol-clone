...
use App\Mail\AllSettlementMail;
...
class AllSettlement extends Command
{
     ...
     * @var string
     */
    protected $signature = 'settlement:all {start_date?} {end_date?}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            // Loop through the given array and append all elements to the "13" key
            foreach ($row_data as $key => $value) {
                $newArray["13"] = array_merge($newArray["13"], $value);
            }

            foreach($newArray as $product_id => $values) {
                $filename = "Consolidated_report_{$start}.xlsx";
            ...
            Mail::to(config('setting.settlement.howden.email_to'))
                ...
                ->send(new AllSettlementMail($filenames, $data));

            Log::info("[Cron - Howden Consolidation Settlement] {$rows} records processed. [{$start_date} to {$end_date}]");

            CronJobs::updateOrCreate([
                'description' => 'Send Consolidation Settlement Report to Insurers',
