...
class ZurichTakaful implements InsurerLibraryInterface
{
    private string $host;
	...
	public function __construct(int $insurer_id, string $insurer_name)
    {
		...
		$this->host_vix = config('insurer.config.zurich_takaful_config.host_vix');
    ...
    private function vehInputMake(object $input) : object
    {
        ...
        $xml = view('backend.xml.zurichTakaful.veh_input_make')->with($data)->render();
        // Call API
    ...
    private function getModelDetails(object $input) : object
    {
        ...
        // Generate XML from view
        $xml = view('backend.xml.zurichTakaful.veh_input_model')->with($data)->render();
        // Call API
        $result = $this->cURL($path, $xml);
    ...
    public function getQuotation(object $input) : object
    {
        ...
        // Generate XML from view
        $xml = view('backend.xml.zurichTakaful.calculate_premium')->with($data)->render();
        // Call API
        $result = $this->cURL($path, $xml);
        if (!$result->status) {
            return $this->abort($result->response, $result->code);
    ...
    public function issue_cover_note(object $input) : object
    {
        ...
        $response['CoverNoteInfo']['CoverNoteError'] = $xml_data->CoverNoteInfo->CoverNoteError ?? '';
        return new ResponseData([
            'response' => (object)$response
        ]); 
    }
    ...
    public function resend_cover_note(object $input) : object
    {
        ...
        // Generate XML from view
        $xml = view('backend.xml.zurichTakaful.resend_cover_note')->with($data)->render();
        // Call API
        $result = $this->cURL($path, $xml);
        if (!$result->status) {
            return $this->abort($result->response, $result->code);
    ...
    public function jpj_status(object $input) : object
    {
        ...
        $agent_code = $this->agent_code;
        $cover_note_no = $input->cover_note_no;//'D02940-18000169';

        $data["agent_code"] = $agent_code;
        $data["cover_note_no"] = $cover_note_no;
        // Generate XML from view
        $xml = view('backend.xml.zurichTakaful.jpj_status')->with($data)->render();
        // Call API
        $result = $this->cURL($path, $xml);
        if (!$result->status) {
            return $this->abort($result->response, $result->code);
        ...
        $response['JPJStatus']['JPJStatusError'] = $xml_data->JPJStatus->JPJStatusError ?? '';
        return new ResponseData([
            'response' => (object)$response
        ]); 
    }
    ...
    private function getVIXNCD(array $input) : ResponseData
    {
        ...
        $data["data"] = "{'AgentCode':'".$this->agent_code.
            ...
            "','Signature':'".$this->generateSignature($signature, $sign_type)."'}";
        // Generate XML from view
        $xml = view('backend.xml.zurichTakaful.veh_input_info')->with($data)->render();
        // Call API
        $result = $this->cURL($path, $xml);
        if(!$result->status) {
            return $this->abort($result->response);
