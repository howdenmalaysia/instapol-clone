        ...
        try {
            ...
            foreach($input->extra_cover as $extra_cover) {
                InsuranceExtraCover::create([
                    ...
                    'cart_amount' => $extra_cover->cart_amount ?? null,
            ...
            Log::info("[Session/Motor/Promo] Received Request: " . json_encode($motor));
            Log::info("[Session/Input/Promo] Received Request: " . json_encode($input));
            if(!empty($input->promo)) {
                Log::info("[API/Promo] Delete Table". json_encode($insurance));
                // InsurancePromo::where([
                //     'insurance_id' => $insurance->id
                // ])->delete();
                Log::info("[API/PROMO/Exits] Request Promo Motor: " . json_encode($input));
                Log::info("[API/PROMO/Exits] Request Promo Motor1: " . json_encode($motor));
...
public function callback(Request $request) 
    {
        ...
        try {
            ...
            InsuranceRemark::create([
                'insurance_id' => $insurance->id,
                'remark' => "{$product->insurance_company->name} Policy successfully created. (Policy Number: {$request->PolicyNumber})",
            ]);
