...
@section('title', implode(' | ', [config('app.name'), __('frontend.motor.payment_failure_page.payment_failed')]))
        ...
        <x-static-page>
            <x-slot name="title">
                <div class="text-center text-uppercase text-white fw-bold about-title">
                    {{__('frontend.motor.payment_failure_page.payment_failed')}}
                </div>
            </x-slot>
            ...
            <x-slot name="content">
                <div class="row align-items-center">
                    <div class="col-6">
                        <div class="px-3 py-4">
                            ...
                            @foreach (__('frontend.motor.payment_failure_page.messages') as $message)
                    ...
                    <div class="col-6">
                        <table class="table table-bordered">
                            <tr>
                                <th class="text-center py-3 w-50">{{ __('frontend.motor.payment_failure_page.insurance_company') }}</th>
                                <td class="text-center py-3">{{ $insurance->product->insurance_company->name }}</td>
                            </tr>
                            <tr>
                                <th class="text-center py-3">{{ __('frontend.motor.payment_failure_page.transaction_number') }}</th>
                                <td class="text-center py-3">{{ $insurance->insurance_code }}</td>
                            </tr>
                            <tr>
                                <th class="text-center py-3">{{ __('frontend.motor.payment_failure_page.total_premium') }}</th>
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        <p class="fw-bold text-center">{{ __('frontend.motor.payment_failure_page.you_might_interested') }}</p>
                        <div class="row">
                            <div class="col">
                                <a href="">
...
@push('after-scripts')
    <script>
        $(() => {
            ...
            gtag('event', 'l_motor_failed', { 'debug_mode': true });
        });
    </script>
@endpush
